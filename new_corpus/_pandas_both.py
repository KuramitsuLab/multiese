...
@test(df=ds=missing;alist=['A','B'];$$)
@alt(置き換える|置換する|変更する)
{__Y__[の値|]を|dictで|まとめて}置き換える
'''

__X__.replace(pattern, s, regex=True)
'''
@test(df=ds=missing;pattern='.*';repl='\1';alist=['A','B'];$$)
@alt(置き換える|置換する|変更する)
{__Y__[の値|]を|正規表現[pattern|]で|まとめて|[sに|]}置き換える
'''

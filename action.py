...
def transform_verb_and_then(s, option, factor=-1):
    if s.endswith('する'):
        return s[:-2]+alt('して、|し、', option, factor)
    if s.endswith('ずる'):  # 論ずる　
        return s[:-2]+alt('じて、|じ、', option, factor)
    if isGroup2Verb(s):  # グループ２ # ラ行と区別が難しい
        return s[:-1]+alt('、|、|て、', option, factor)
    if s.endswith('く'):  # 書く
        return s[:-1]+alt('き、|いて、', option, factor)
    if s.endswith('す'):  # 探す
        return s[:-1]+alt('し、|して、', option, factor)
    if s.endswith('つ'):  # 勝つ
        return s[:-1]+alt('ち、|って、', option, factor)
    if s.endswith('ぬ'):  # 死ぬ
        return s[:-1]+alt('に、|んて、', option, factor)
    if s.endswith('む'):  # 読む
        return s[:-1]+alt('み、|んで、', option, factor)
    if s.endswith('る'):  # 切る   ラ行
        return s[:-1]+alt('り、|って、', option, factor)
    if s.endswith('う'):  # 笑う
        return s[:-1]+alt('い、|って、', option, factor)
    if s.endswith('ぐ'):  # 防ぐ
        return s[:-1]+alt('ぎ、|いで、', option, factor)
    if s.endswith('ぶ'):  # 遊ぶ
        return s[:-1]+alt('び、|んで、', option, factor)
    return None


def transform_verb_and_noun(s, option, factor=-1):
    if s.endswith('する'):
        return s[:-2]+alt('した|した|された', option, factor)
    if s.endswith('ずる'):  # 論ずる　
        return s[:-2]+alt('じた|じた|じられた、', option, factor)
    if isGroup2Verb(s):  # グループ２ # ラ行と区別が難しい
        return s[:-1]+alt('た|た|られた', option, factor)
    if s.endswith('く'):  # 書く
        return s[:-1]+alt('いた|いた|かれた', option, factor)
    if s.endswith('す'):  # 探す
        return s[:-1]+alt('した|した|された', option, factor)
    if s.endswith('つ'):  # 勝つ
        return s[:-1]+alt('った|った|たれた', option, factor)
    if s.endswith('ぬ'):  # 死ぬ
        return s[:-1]+alt('んだ|んだ|なれた', option, factor)
    if s.endswith('む'):  # 読む
        return s[:-1]+alt('んだ|んだ|まれた', option, factor)
    if s.endswith('る'):  # 切る   ラ行
        return s[:-1]+alt('った|った|られた', option, factor)
    if s.endswith('う'):  # 笑う
        return s[:-1]+alt('った|った|われた、', option, factor)
    if s.endswith('ぐ'):  # 防ぐ
        return s[:-1]+alt('いだ|いだ|がれた', option, factor)
    if s.endswith('ぶ'):  # 遊ぶ
        return s[:-1]+alt('んだ|んだ|ばれた', option, factor)
    return None
...
def perform_noun(pairs, option):  # 名詞に変える
    pairs_noun = []
    for sentence, code in pairs:
        sentence_and_noun = transform_verb_and_noun(
            sentence, '', option, factor=2)
...
def perform_let(pairs, option):  # 代入文に変える
    ...
    for sentence, code in pairs:
        code_let = f'X = {code}'
        sentence_and_then = transform_verb_and_then(sentence, option, factor=6)
        if sentence_and_then:
            sentence_let = sentence_and_then + 'X' + \
                optional_choice(option, 'にする|とする|に代入する')
        ...
        sentence_let = sentence + 'をX' + \
            optional_choice(option, 'にする|とする|に代入する')
        pairs_let.append((sentence_let, code_let))
        if sentence.endswith('かどうか'):
            # 小さいかどうか　=> 小さいとき、Xを真とする
            s, _ = remove_whether(sentence)
            sentence_let = s + 'とき、Xを真' + \
                optional_choice(option, 'にする|とする', seed=1)
            pairs_let.append((sentence_let, code_let))
    return pairs_let
...
def perform_let_self(pairs, option):
    ...
    for sentence, code in pairs:
        ...
        sentence_and_then = transform_verb_and_then(
            sentence, random_seed(option))
        if sentence_and_then:
            sentence_let_self = sentence_and_then + name + \
                optional_choice(option, 'にする|とする|に代入する')
            pairs_let_self.append((sentence_let_self, code_let_self))
            sentence_let_self = sentence_and_then + \
                optional_choice(option, '置き換える|再代入する')
            pairs_let_self.append((sentence_let_self, code_let_self))
            sentence = transform_verb_and_noun(sentence, '結果', 0)
        sentence_let_self = sentence + 'を' + name + \
            optional_choice(option, 'にする|とする|に代入する')
        pairs_let_self.append((sentence_let_self, code_let_self))
...
def perform_inplace(pairs, option):
    ...
    for sentence, code in pairs:
        ...
        sentence_and_then = transform_verb_and_then(
            sentence, random_seed(option))
        if sentence_and_then:
            sentence_inplace = sentence_and_then + \
                optional_choice(option, 'インプレースする|置き換える|書き換える')
            pairs_inplace.append((sentence_inplace, code_inplace))
            sentence = transform_verb_and_noun(sentence, '結果', 0)
        sentence_inplace = sentence + \
            optional_choice(option, 'でインプレースする|で置き換える|で書き換える')
        pairs_inplace.append((sentence_inplace, code_inplace))
...
def perform_then(pairs, option):
    ...
    if option.get('partial', True):
        for sentence, code in pairs:
            verb = transform_verb_and_then(sentence, option)
            if verb:
